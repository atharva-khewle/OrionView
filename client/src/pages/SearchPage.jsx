...
export const SearchPage = () => {
  ...
  const handlePageChange = async (newPage) => {
    const response = await axios.get(`http://${currentHost}/search/?content=${contentType}&query=${currQ}&page=${newPage}`);
  ...
  const handleSearch = async () => {
    ...
    try {
      if (currQ !== query) {
        setPage(1);
        const response = await axios.get(`http://${currentHost}/search/?content=${contentType}&query=${query}&page=1`);
        ...
        const response = await axios.get(`http://${currentHost}/search/?content=${contentType}&query=${query}&page=${Page}`);
